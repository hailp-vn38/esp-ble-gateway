                       ...
                       const device_schema_feature_t *filter)
{
    ...
    features = NULL;
    if (filter != NULL) {
        if (!mcp_sem_serialize_feature(cJSON_GetObjectItem(payload, "features"),
                                       schema, filter))
            goto fail;
    } else {
        for (size_t i = 0; i < schema->feature_count; ++i) {
            if (!mcp_sem_serialize_feature(
                    cJSON_GetObjectItem(payload, "features"), schema,
                    &schema->features[i]))
                goto fail;
        }
    }
                                     ...
                                     mcp_device_control_plan_t *out)
{
    ...
    mcp_sem_resolve_status_t device_status = mcp_sem_resolve_device(
        cJSON_GetObjectItemCaseSensitive(params, "device"), out->device_id,
        sizeof(out->device_id));
    if (device_status == MCP_SEM_AMBIGUOUS)
        set_tool_error(out, "ambiguous_device", protocol);
    else if (device_status != MCP_SEM_OK)
    ...
    if (has_feature) {
        mcp_sem_resolve_status_t status =
            mcp_sem_resolve_feature(&schema, feature_arg, &feature);
        if (status == MCP_SEM_AMBIGUOUS)
            set_tool_error(out, "ambiguous_feature", protocol);
        else if (status != MCP_SEM_OK)
