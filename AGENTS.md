...
- BLE wire format is CBOR with numeric map keys; schema lives in `components/cbor_codec/cbor_codec.c` (protocol version 1). Peripheral devices implement service `0xABF0`, char `0xABF1` (write), `0xABF2` (notify).
- Components: `device_store` (NVS registry), `wifi_provisioning`, `ble_central` (NimBLE), `cbor_codec`, `command_dispatcher` (registry + per-device ACK routing), `web_server`, `mcp_endpoint` (JSON-RPC subset at `POST /mcp`, no auth — LAN only), `log_buffer`, `qcbor_lib` (submodule wrapper).
